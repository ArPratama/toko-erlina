...
<button
  onclick="loadModal('users/form','fetchRole()')"
...
<table id="tblusers" class="w-full whitespace-nowrap">
  <thead>
...
<script>
  ...
  $('#tblusers').DataTable( {
    ajax: {
      url: apiUrl+'/users/get?_s='+getCookie(MSG['cookiePrefix']+'AUTH-TOKEN'),
    },
    "ordering": false,
    columns: [
      { data:'FullName', className:'px-4 py-3 text-sm' },
        ...
        render: function (data, type, full, meta) {
          var html = '<div class="flex item-center justify-center">' +
                        '<div onclick="showDetailForm(\''+full['ID']+'\')" class="w-4 mr-2 transform hover:text-blue-500 hover:scale-110 cursor-pointer">' +
                          ...
                          '</svg>' +
                        '</div>' +
                        '<div onclick="showDeleteConfirm(\''+full['ID']+'\',\''+full['FullName']+'\')" class="w-4 mr-2 transform hover:text-blue-500 hover:scale-110 cursor-pointer">' +
                          '<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">' +
  ...
  function doSearchTable() {
    $('#tblusers').DataTable().search( $('#txtSearch_tblusers').val() ).draw();
  }

  function showDetailForm(ID) {
    loadModal('users/form','onDetailForm(\''+ID+'\')');
  }
