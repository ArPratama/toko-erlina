<div class="float-left">
  ...
  />
  <input
    id="txtFilterEndTransactionDate"
    name="txtFilterEndTransactionDate"
    type="date"
    maxlength="50"
...
<script>
  ...
  function initDataTable() {
      ...
      $('#tblreports').DataTable( {
        ajax: {
          url: apiUrl+'/reports/getPurchasing',
          data: function(d) {
            d.startTransactionDate = $('#txtFilterStartTransactionDate').val();
            d.endTransactionDate = $('#txtFilterEndTransactionDate').val();
        ...
        columns: [
          ...
          { data:'TotalItem', className:'px-4 py-3 text-sm' },
          {
            data:'TotalPrice',
  ...
  function ShowAllData() {
     $('#txtFilterStartTransactionDate').val("");
     $('#txtFilterEndTransactionDate').val("");
     $('#tblreports').show();
     initDataTable();
    }

  function showDetailView(ID) {
    loadPage('report_purchasing/detail/index','onDetailForm(\''+ID+'\')');
  }
